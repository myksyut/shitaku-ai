...
```bash
git clone https://github.com/emuni-kyoto/ai-coding-project-boilerplate-fastapi-nextjs.git my-awesome-project
cd my-awesome-project
rm -rf .git  # ボイラープレートのgit履歴を削除
...
すでにプロジェクトがある場合は、必要なファイルをコピーして使うこともできます。まず、ボイラープレートを別の場所にダウンロードします。

```bash
# 一時的にボイラープレートをダウンロード
git clone https://github.com/emuni-kyoto/ai-coding-project-boilerplate-fastapi-nextjs.git temp-boilerplate
```
...
まずは設計書を作り、それをユーザーとレビューし、認識を揃える。その設計書をコンテキストとして作業を計画し、タスクを作る。タスクの実行は専用のコンテキストを持つサブエージェントが担うことで、タスクに不要な情報が入らず実装が安定する。このような仕組みにすることで、コーディングエージェント単体ではコンテキストウィンドウに収まらないような規模な開発でも品質を保ちながら進められるようになります。
